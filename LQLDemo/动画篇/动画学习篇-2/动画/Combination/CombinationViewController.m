...
- (void)dingdingAnimation {
    ...
    CGFloat upMenuViewX = kWidth - homeLabel.QL_width - 20.f;
    CGFloat upMenuViewY = kHeight - homeLabel.QL_height - 20.f;
    DWBubbleMenuButton *upMenuView = [[DWBubbleMenuButton alloc] initWithFrame:CGRectMake(upMenuViewX,upMenuViewY,homeLabel.QL_width,homeLabel.QL_height) expansionDirection:DirectionUp];
...
- (void)dianzanAnimation {
    MCFireworksButton *btn = [[MCFireworksButton alloc] initWithFrame:CGRectMake(kWidth/2-50, kHeight/2-50, 100, 100)];
...
- (void)bezierPathAnimation {
    ...
    // 首先设置一个起始点
    CGPoint startPoint = CGPointMake(kWidth/2, 220);
    // 以起始点为路径的起点
    [path moveToPoint:CGPointMake(kWidth/2, 220)];
    // 设置一个终点
    CGPoint endPoint = CGPointMake(kWidth/2, kHeight - 300);
    // 设置第一个控制点
    ...
    // 设置第三个控制点
    CGPoint controlPoint3 = CGPointMake(kWidth, 280);
    // 设置第四个控制点
    CGPoint controlPoint4 = CGPointMake(kWidth - 100, 120);
    // 添加二次贝塞尔曲线
...
- (LJInstrumentView *)progressView {
    if (!_progressView) {
        ...
        _progressView.QL_centerX = kWidth/2;
